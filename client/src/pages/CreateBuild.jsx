...
const CreateBuild = () => {
  ...
  const handleSubmit = async (e) => {
	...
	console.log("Submitting build:", finalBuild);

	await BuildsAPI.createBuild(finalBuild);
  };


  const yourBuild = Object.entries(selectedOptions)
  	.filter(([tabId, optionId]) => !!optionId)
	.map(([tabId, optionId]) => {
		...
		<section className="options-section">
		  ...
		  {selectedTab !== "your-build" ? (
				...
				<ul>
				  {yourBuild.map((item) => (
					<li key={`${item.tab}-${item.id}`}>
					  <strong>{item.tab.toUpperCase().slice(0,-1)}: </strong> {item.name} ($
					  {item.price})
					</li>
				  ))}
				</ul>
			...
			<div className="your-build-footer">
				...
				<label htmlFor="build-name">Name of your new build:</label>
				<input
					type="text"
					required
					id="build-name"
					...
					onChange={(e) =>
					setBuildData((prev) => ({ ...prev, name: e.target.value }))
					}
				/>
				</div>
